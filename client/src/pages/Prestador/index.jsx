...
const Prestador = () => {
    ...
    if (!userProfile || userProfile.role != api.ROLE_PROVIDER) return <Unauthorized />

    const services = useQuery({
        queryKey: [api.QUERY_SERVICES, filter],
        queryFn: async () => await api.getServices({search: filter, owner: userProfile.id}),
    })
    ...
    return <>
        {(services.isPending || editService.isPending || categories.isPending) && <Load />}
        {!services.isError ? (
            <Div as={"section"} $flex gap={"10px"}>
                ...
                <Div $flex gap={'8px'}>
                    {services.isSuccess && categories.isSuccess && services.data.length > 0
                        ? services.data.map((item, key) => (
                            <CardService onClick={() => setModal(item)}
                                key={key}
                                data={item}
                                dataset={{
                                    owner: {name: userProfile.name},
