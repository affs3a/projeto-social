...
from .serializers import SolicitationSerialize
...
class SolicitationList(APIView):
    def get(self, request, format=False):
        ...
        serializer = SolicitationSerialize(queryset, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def put(self, request, format=False):
        serializer = SolicitationSerialize(data=request.data)
...
class SolicitationDetails(APIView):
    ...
    def get(self, request, solicitation_id, format=False):
        ...
        serializer = SolicitationSerialize(Solicitation)
        return Response(serializer.data, status=status.HTTP_200_OK)
